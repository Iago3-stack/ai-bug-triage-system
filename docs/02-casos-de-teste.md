...
Casos de teste documentados em **Gherkin** (padrão BDD) para validar o motor de triagem. Estes casos espelham os testes automatizados de `test_triagem.py` (motor: **18 testes**), que rodam no **CI** (GitHub Actions) a cada push junto com os demais (Jira, persistência, guardrails, dashboard, RAG e nuvem — **82 no total**).
...
Resultado esperado: **82 passed** (18 do motor + 15 do Jira + 8 da persistência + 14 dos guardrails + 6 do dashboard + 10 do RAG + 10 da persistência em nuvem — também validado automaticamente pelo CI).
