...
| **Cobertura de testes** | **475 testes** `pytest` — motor (18), **colar falha (13)**, **adaptadores Playwright/Postman (16)**, Jira (15), persistência (14), guardrails (18), dashboard (17), RAG (23), nuvem/Supabase (14), Pix (10), IA (36), Auth/Supabase (46), notificações (37), plano (25), cobrança Pix (21), perfil (16), **GitHub (18)**, **painel do dono (37: Teste Premium com validade via `teste_ate`, `definir_trial`/`definir_plano_manual`, limpeza do teste ao pagar via PATCH, upsert da tabela `usuarios`, `eh_dono` e o probe `teste_disponivel` que explica o 400 PGRST204 enquanto a coluna não está no schema cache)**, persistência de sessão (8), ferramenta (3), **webhook de CI (28: payloads válidos/inválidos, teto 200 KB, token, IA/persistência opcionais, confirmação automática de pagamento e transporte HTTP real)**, **PagBank (22: pedido com QR dinâmico, CPF, `notification_urls`, expiração e consulta de status)**, **landing/Pages (12: domínio próprio, canonical, sitemap, badge)**, **legal (5: Termos/Privacidade)** e **upload de foto do perfil (3: JPG/PNG/WebP abrem e HEIC sem pillow-heif dá recado amigável sem quebrar)** — cobrindo severidade, negação, sentimento, determinismo, exportação, fuso, PII, agregações de QA, recuperação do histórico, persistência em nuvem, registro de resolução (aprendizado do RAG), ranking por BM25 com sinônimos e ponderação por recência/resolução, rerank vetorial híbrido com vetores mockados e fallback para o lexical sem chave de embeddings, geração de QR Pix (EMV/CRC), fallback Gemini → Groq (open-weight), seletor de provedor de IA (automático/forçado), **modelo próprio** (OpenAI-compatível com retry sem JSON mode + Gemini custom com chave específica), **auth/Supabase** (cadastro/login/logout, isolação por sessão), **notificações** (e-mail SMTP + Discord com testadores à prova de exceção, **avisos genéricos de evento sem template de triagem**, **erro real do SMTP exposto no "✉️ Testar e-mail" e nos alertas via `ultimo_erro_email()`**), **planos** (Basic/Premium com gating de recursos, **plano por usuário no banco**, **novo usuário com nuvem ativa sempre começa em Basic**, migração de registros legados "global" → uid e **Teste Premium que expira sozinho**), **cobrança Pix** ("nosso Stripe": geração de cobrança, confirmação que ativa Premium, cancelamento, estorno que volta a Basic, preço configurável e **payload da assinatura priorizando PIX_COPIA_PLANO**), **perfil do usuário** (nome/empresa/fuso/avatar com fallback JSONL, **nuvem ganha do local**, fuso inválido → padrão e iniciais para a bolinha), **GitHub por usuário** (criação de issues no repositório do próprio usuário via `api.github.com`, token/repo por sessão, normalização de URL e mensagens de erro amigáveis), **colar falha** (Pilar 1: extração de título/categoria/módulo/versão/erro/local de stack traces Python, Java e JS e severidade prévia via motor local), **adaptadores Playwright/Postman** (Pilar 2: saída de teste Playwright — nome, `Error: expect(...)`, Expected/Received, arquivo:linha, texto ou JSON do relator; e Postman/newman — método+URL, **HTTP esperado × recebido**, asserção e erro do corpo JSON, com linhas "Ferramenta de origem"/"Requisição" no relato e fallback para o parser genérico), **webhook de CI** (Pilar 3: `POST /webhook/falha` converte a evidência do CI no relato pronto via adaptadores, com token `WEBHOOK_TOKEN`, IA e persistência opcionais, e a action `ci-falhas.yml` reporta automaticamente quando o teste do CI falha), **painel do dono** (Teste Premium com validade, registro automático de contas e ações de admin) e **persistência de sessão** (ponte cookie → iframe) |
...
- ~~**Cobrança automática via PagBank (Pix)**~~ ✅ — QR dinâmico pela API de Pedidos com confirmação automática via `/webhook/pagamento` (idempotente); produção aguardando whitelist da conta.
...
- ~~**RAG no histórico**~~ ✅ — retrieval híbrido BM25 + vetores (sinônimos, recência, resolução) + geração; resposta "já aconteceu? como resolvemos?" com resolução registrada pelo usuário.
