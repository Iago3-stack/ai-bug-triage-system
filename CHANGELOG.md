...
### Adicionado
- **⚙️ Configurações unificadas (v2.5.6)** — o sidebar ganhou um expander único **"⚙️ Configurações"**: o **seletor de tema** (☀️/🌙) e a **configuração do Jira** migraram pra dentro. O **Jira** vira um sub-expander **🔑** com borda azul persistente (visível no tema claro, sem depender de hover), deixando claro onde ficam os campos. A sidebar fica limpa e pronta pra receber futuras opções (notificações, modelo padrão, etc.) sempre nesse mesmo lugar.
...
- **🔑 Botão "Salvar configuração (sessão)" verde fixo (v2.5.6)** — no sub-expander Jira do sidebar, o botão que salva as credenciais ganhou **verde fixo `#059669`** (texto branco) e **não muda no hover**, tanto no tema claro quanto no escuro (vencendo a regra escura do chevron, que o deixava transparente/cinza lá).
