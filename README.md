...
- 🧪 **106 testes + CI** — suíte `pytest` (motor, Jira, persistência, guardrails, dashboard, RAG, nuvem, Pix e IA) rodando a cada push via GitHub Actions (badge de qualidade em cima).
...
- ✅ **Testes unitários do motor (`pytest`)** — 106 testes (motor + Jira + persistência + guardrails + dashboard + RAG + nuvem + Pix + IA), rodam automaticamente via CI (GitHub Actions)
